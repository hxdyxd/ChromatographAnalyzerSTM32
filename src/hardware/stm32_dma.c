...
struct sDmaCfg DMA_CFG[DMA_NUM] = {
    {
        #define DMA_IRQHandler_Index0   DMA1_Channel2_IRQHandler
        #define DMA_FLAG_TC_Index0      DMA1_FLAG_TC2
        .RCC_AHBPeriph = RCC_AHBPeriph_DMA1,
        .DMAy_Channelx = DMA1_Channel2,   //USART2_TX
        .DMA_PeripheralBaseAddr = (uint32_t)(&(USART3->DR)),  //USART2
...
int stm32_dma_usart_write(uint8_t *p, uint32_t len)
{
